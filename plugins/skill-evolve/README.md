...
`~/.claude-plugins/investigate/`・`~/.claude-plugins/dev/`・`~/.claude-plugins/repo-know/`・`~/.claude-plugins/prompt-insight/` に蓄積されたデータを分析し、スキルを改善する `/skill-evolve` コマンドを提供します。

```
/skill-evolve を実行すると...

investigate_bug:
  ~/.claude-plugins/investigate/bug/ の未解決不明点パターン
  + ~/.claude-plugins/repo-know/<project>/ のプロジェクト知識
    ↓
  investigate_bug スキルのヒアリング項目・ガードレールを改善（EVOLVE セクション更新）

investigate_code:
  ~/.claude-plugins/investigate/code/ の調査ログ
  + ~/.claude-plugins/repo-know/<project>/ のプロジェクト知識
    ↓
  プロジェクトごとのスキル拡張ファイルを更新（gitignore 済み）

dev:
  ~/.claude-plugins/investigate/bug/ + ~/.claude-plugins/dev/ を横断集計
    ↓
  dev スキルの計画・実行フェーズを改善（EVOLVE セクション更新）

共通: ~/.claude-plugins/prompt-insight/ のプロンプトログ（直近30日）
    ↓
...
└── scripts/
    ├── collect_investigate.py            # ~/.claude-plugins/investigate/bug/ の未解決パターンを集計
    ├── collect_investigate_code.py       # ~/.claude-plugins/investigate/code/ + ~/.claude-plugins/repo-know/ を集計
    ├── collect_cross_skill.py            # investigate/bug + dev を横断集計
    └── collect_prompt_insight.py         # ~/.claude-plugins/prompt-insight/ のログを集計
```
...
| ディレクトリ | 書き込むスキル | 内容 |
|-------------|-------------|------|
| `~/.claude-plugins/investigate/bug/<project>/` | investigate_bug | バグ調査ログ・未解決不明点 |
| `~/.claude-plugins/investigate/code/<project>/` | investigate_code | コード調査ログ |
| `~/.claude-plugins/repo-know/<project>/` | investigate_bug・investigate_code | プロジェクト固有の知識（tech/domain/decisions） |
| `~/.claude-plugins/dev/<project>/` | dev | タスク管理ログ |
| `~/.claude-plugins/prompt-insight/logs/` | （フック自動記録） | プロンプトログ |
...
| `~/.claude-plugins/skill-evolve/skill-extensions/<project>/investigate_code.local.md` | investigate_code のプロジェクト別拡張（**主な出力**） | `/skill-evolve` 実行時に自動生成・更新 |
...
`~/.claude-plugins/prompt-insight/logs/` が存在しない場合は自動的にスキップします。セットアップは `/prompt-insight setup` で行えます。
