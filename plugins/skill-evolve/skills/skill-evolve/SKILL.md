...
```
~/.skill-evolve/skill-extensions/<project>/investigate_code.local.md
```
...
全プロジェクトのデータ（`~/.repo-know/` 以下）は収集スクリプトがすでに読み込み済み。
スキル改善を通じて新たに判明したプロジェクト知識があれば、該当プロジェクトの `~/.repo-know/<project>/` に追記する。
