...
description: |
  ...
  ~/.claude-plugins/investigate/ の未解決パターンから investigate_bug スキルを改善し、
  ~/.claude-plugins/dev/ の保留タスクから dev スキルを改善する。
  ~/.claude-plugins/investigate/code/ のドメイン知識から investigate_code のスキル拡張ファイルを更新する。
---

# skill-evolve

`~/.claude-plugins/investigate/` と `~/.claude-plugins/dev/` に蓄積されたデータ、および `~/.claude-plugins/prompt-insight/` のプロンプトログを分析し、スキルを直接改善する。
...
`~/.claude-plugins/prompt-insight/logs/` が存在しない場合はスキップ（エラーにしない）。
...
```
~/.claude-plugins/skill-evolve/skill-extensions/<project>/investigate_code.local.md
```

ディレクトリが存在しない場合は `mkdir -p` で作成する。

更新内容は `~/.claude-plugins/repo-know/<project>/` と `summary.md` から抽出した以下の情報:
- アーキテクチャ・調査の起点
- ドメイン用語・注意点
- 繰り返し調査されている領域
...
全プロジェクトのデータ（`~/.claude-plugins/repo-know/` 以下）は収集スクリプトがすでに読み込み済み。
スキル改善を通じて新たに判明したプロジェクト知識があれば、該当プロジェクトの `~/.claude-plugins/repo-know/<project>/` に追記する。
