...
今回の調査で得られたプロジェクト固有の知識を `~/.investigate/code/<project>/domain-knowledge.md` に追記する。

既存ファイルがあれば読み込んで重複しない内容のみ追記する。なければ新規作成する。

抽出する内容:
- アーキテクチャ・レイヤー構成（例: Controller → Service → Repository の対応）
- よく調査される領域と調査の起点（例: 認証は AuthMiddleware から入ると早い）
- ドメイン固有の用語・概念
- 注意点・落とし穴（例: User.status は enum ではなく文字列定数）
